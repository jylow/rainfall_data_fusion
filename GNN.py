...
def train_epoch(model, data, dataloader, optimizer, device):
  ...
  gen_mask = torch.tensor(data['general_station'].train_mask, dtype=torch.bool).to(device)
  rain_mask = torch.tensor(data['rainfall_station'].train_mask, dtype=torch.bool).to(device)
  ...
  for batch in tqdm.tqdm(dataloader, desc="training"):
    ...
    for i in range(batch_size):
      ...
      training_loss = F.mse_loss(gen_predictions, gen_y[i][gen_mask]) + F.mse_loss(rain_predictions, rain_y[i][rain_mask])
