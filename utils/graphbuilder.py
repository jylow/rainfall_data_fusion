...
class GraphBuilder:
  ...
  def __init__(self, node_feature_dict: dict, station_lists = dict):
    ...
    for nodetype, nodedata in node_feature_dict.items():
      self.hetero_data[nodetype].x = torch.tensor(np.array(nodedata).transpose(1, 0, 2), dtype=self.dtype)
      self.hetero_data[nodetype].y = torch.tensor(np.array(nodedata)[:,:,0:1].transpose(1,0,2), dtype=self.dtype)
  ...
  def get_training_graph(self) -> HeteroData:
    ...
    for nodetype, nodedata in self.hetero_data.node_items():
      ...
      training_mask = [True if stn in training_stations else False for stn in stations]
